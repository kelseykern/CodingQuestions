...
vector <int> findPositions(vector <int> arr, int x) {
  queue<valueIndex*> q1; //value index
  queue<valueIndex*> q2;
  valueIndex *largestPair = new valueIndex(-1,0);
  vector<int>indices;
  
  //create q1 which has values and their index
  int i = 0;
  for (const auto& e: arr){
     valueIndex * v = new valueIndex(e,i);
     q1.push(v);
     i++;
  }
  
 while(q1.size() > 1 && indices.size() < x){ //indices is acting as a counter too
    int popped = 0;
    largestPair->val = -1;
    largestPair->index = 0;
    while(popped < x && q1.size()>0){
        //pop x amount from q1 and push to q2 and keep track of largest
        if( q1.front()->val > largestPair->val) largestPair = q1.front();
        q2.push(q1.front());
        q1.pop();
        popped++;
    }
   
    //add largest 1 based value to indices
    indices.push_back(largestPair->index + 1); //based 1 indexing
    
    //push all from q2 to q1 except largest
    while(!q2.empty()){
      if(q2.front()->index == largestPair->index){
        q2.pop();
      }
      else
      {
        //if the value is 0, leave as 0, otherwise subract 1 from it
        if((q2.front()->val) > 0) q2.front()->val = (q2.front()->val) - 1;
        q1.push(q2.front());
        q2.pop();
       }
    }
